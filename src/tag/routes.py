...
async def update_tag(tag_id: int, tag_data: TagSchemaUpdateRequest, session: AsyncSession = Depends(database)):
    tag = await TagRepository.update(tag_id, tag_data, session)
    if tag is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tag not found.")
    return tag

@router.delete("/{tag_id}", response_model=None)
async def delete_tag(tag_id: int, session: AsyncSession = Depends(database)):
