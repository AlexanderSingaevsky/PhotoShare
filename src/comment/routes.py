...
from src.comment.schemas import CommentResponseSchema, CommentSchema, CommentUpdateSchema, CommentUpdateResponseSchema
...
async def get_all_comments(limit: int = Query(default=10, le=200), db: AsyncSession = Depends(database)):
...
async def get_one_comment(comment_id: int = Path(ge=1), db: AsyncSession = Depends(database)):
...
async def create_comment(body: CommentSchema, db: AsyncSession = Depends(database)):
...
async def update_comment(body: CommentUpdateSchema, db: AsyncSession = Depends(database)):
...
async def delete_comment(comment_id: int = Path(ge=1), db: AsyncSession = Depends(database)):
